/*
    ...
    Homework 8 - Sliders and Tabs
*/

// Followed Examples and documentation here: http://api.jqueryui.com/slider/
...
$(function () 
{
   ...
   makeSliders();
    ...
    $("#form").validate(
    {
        ...
        submitHandler: submitFunc  // When user clicks submit in form
...
{
    // Each slider should have the same values
    // Followed the PDF provided for this style
    var sliderOpts = {
        ...
        // When a slider slides
        slide: function() {

            //Get the sliders value and put it at the appropriate place in the form
            var min_multpcnd_num = $("#min_Multcnd_slider").slider("value"); 
            $("#min_Multiplicand").val( min_multpcnd_num );

            var max_multpcnd_num = $("#max_Multcnd_slider").slider("value");
            $("#max_Multiplicand").val( max_multpcnd_num );

            var min_multplr_num = $("#min_Multplr_slider").slider("value");
            $("#min_Multiplier").val( min_multplr_num );

            var max_multplr_num = $("#max_Multplr_slider").slider("value");
            $("#max_Multiplier").val( max_multplr_num );
            
            submitFunc(); // Generate the table
        },  
    };

    $( "#min_Multcnd_slider" ).slider(sliderOpts);

    $( "#max_Multcnd_slider" ).slider(sliderOpts);

    $( "#min_Multplr_slider" ).slider(sliderOpts);

    $( "#max_Multplr_slider" ).slider(sliderOpts);
}